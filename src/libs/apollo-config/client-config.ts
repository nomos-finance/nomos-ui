...
function buildClientQueries(
    ...
    wsLink['subscriptionClient'].onDisconnected(() => {
        ...
        client.writeQuery({
            query: cachedServerWsErrorContext.query,
            data: {
                [cachedServerWsErrorContext.errorKey]: (previous?.[cachedServerWsErrorContext.errorKey] || 0) + 1
            }
        })
    })
    ...
    wsLink['subscriptionClient'].onError(() => {
        const previous = client.readQuery({ query: cachedServerWsErrorContext.query })
        console.log(`WebSocket error occurred (${previous?.[cachedServerWsErrorContext.errorKey] || 0}), retrying...`)
        client.writeQuery({
            query: cachedServerWsErrorContext.query,
            data: {
                [cachedServerWsErrorContext.errorKey]: (previous?.[cachedServerWsErrorContext.errorKey] || 0) + 1
...
export function getApolloClient({ cachingServerUrl, cachingWSServerUrl, protocolDataUrl }: NetworkConfig): {
    client: ApolloClient<NormalizedCacheObject>
    wsClients: any[]
} {
    ...
    const thegraphDataLink =
        governanceDataLink === undefined
            ...
            : split(
                  ({ query }) => {
                      const definition = getMainDefinition(query)
                      return definition.kind === 'OperationDefinition' && definition.operation === 'query' && definition.name?.value === 'UserHistory'
                  },
                  poolTheGraphLink,
                  governanceDataLink
              )
    ...
    const generalLink =
        cachedServerDataLink === undefined
            ...
            : split(
                  ({ query }) => {
                      const definition = getMainDefinition(query)
                      return definition.kind === 'OperationDefinition' && !!definition.name?.value.startsWith('C_')
                  },

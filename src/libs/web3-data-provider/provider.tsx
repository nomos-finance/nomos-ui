...
const formattingError = (error: Error | undefined, supportedNetworks: Network[], intl: IntlShape) => {
...
export function Web3Provider({
    ...
    const [preferredNetwork, setPreferredNetwork] = useState((localStorage.getItem('preferredNetwork') as Network | null) || defaultNetwork)
    ...
    const handleNetworkChange = async (network: Network) => {
        ...
        if (currentProviderName && library) {
            return await handleActivation(currentProviderName, network, supportedNetworks, connectorOptionalConfig)
        }
    }
    const handleUnlockWallet = useCallback(
        async (connectorName: AvailableWeb3Connectors, network: Network, availableNetworks: Network[], connectorConfig: ConnectorOptionalConfig) => {
            ...
            if (!currentValue) {
                setConnectorOptionalConfig(config => {
                    ...
                    if (reloadAccounts && currentProviderName) {
                        handleActivation(currentProviderName, preferredNetwork, supportedNetworks, updatedConfig)
                    }
    ...
    useEffect(() => {
        ...
        safeAppConnector.isSafeApp().then(isSafeApp => {
            ...
            if (storedProviderName) {
                ...
                handleUnlockWallet(storedProviderName, preferredNetwork, supportedNetworks, connectorOptionalConfig)
            } else {
                setCurrentAccount('')
                setActivation(false)
            }

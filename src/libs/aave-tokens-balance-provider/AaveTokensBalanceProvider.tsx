...
const Context = React.createContext<WalletBalanceProviderContext>({} as WalletBalanceProviderContext)
...
export const AaveTokensBalanceProvider: React.FC = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const contract = WalletBalanceProviderFactory.connect(governanceNetworkConfig.walletBalanceProvider, getProvider(governanceConfig.network))
        const fetchAaveTokenBalances = async () => {
            setAaveTokensLoading(true)
            try {
                const balances = await contract.batchBalanceOf(
                    [walletAddress],
                    [governanceConfig.aaveTokenAddress, governanceConfig.aAaveTokenAddress, governanceConfig.stkAaveTokenAddress]
                )
                setAaveTokens({
